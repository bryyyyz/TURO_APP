<template>
  <div class="dashboard-container">
      ...
      <div class="sidebar-footer">
        <div class="user-profile-wrap">
          <div class="user-profile-widget" @click.stop="toggleLogoutMenu">
            ...
            <TuroIcon name="chevronDown" :size="14" class="chevron sidebar-chevron-ic" />
          </div>
      ...
      <header class="content-header desktop-only">
        ...
        <div class="header-right">
          ...
          <div class="search-box-desktop">
            <input type="text" placeholder="Search tutors, subjects, or topics..." />
          ...
          <!-- Book Button -->
          <button type="button" class="btn-book" @click="currentTab = 'discover'">
            <TuroIcon name="calendar" :size="18" class="btn-book-ic" />
            Book a Session
          </button>
        </div>
      </header>
    ...
    <nav class="bottom-nav">
      ...
      <div class="nav-item" :class="{ active: currentTab === 'discover' }" @click="currentTab = 'discover'">
    ...
    <Teleport to="body">
      <div v-if="showLogoutModal" class="logout-modal-shell">
        ...
        <div class="logout-modal" role="dialog" aria-modal="true" aria-label="Log out confirmation">
          ...
          <p>You can always sign back in anytime to continue learning.</p>
...
import StudentSessions from '../components/dashboard/StudentSessions.vue';
...
const dashboardAvatarSrc = computed(() => {
  if (avatarLoadFailed.value) return '/images/student_avatar.png';
  return profile.value?.avatar_url || '/images/student_avatar.png';
});
...
async function loadStudentActivityFeed(forPanel = false) {
  const sid = profile.value?.user;
  if (!sid) return;
  if (forPanel) notificationsLoading.value = true;
  try {
    const [bRes, pRes] = await Promise.all([
      bookingService.getStudentBookings(sid),
      paymentService.getStudentPayments(sid),
    ]);
    ...
    notificationItems.value = buildStudentActivities(bookings, payments);
    notificationBadgeCount.value = Math.min(notificationItems.value.length, 99);
  } catch (e) {
    console.error('[StudentDashboard] activity feed', e);
  } finally {
...
function toggleNotifications() {
  ...
  if (notificationsOpen.value) loadStudentActivityFeed(true);
}

function openNotificationsPanel() {
  notificationsOpen.value = true;
  loadStudentActivityFeed(true);
}
...
const tabs = [
  { id: 'dashboard', label: 'Dashboard', icon: 'layoutGrid', component: StudentOverview },
  { id: 'discover', label: 'Discover Tutors', icon: 'search', component: Discover },
  { id: 'sessions', label: 'Sessions', icon: 'calendar', component: StudentSessions },
  { id: 'messages', label: 'Messages', icon: 'messageSquare', component: Messages },
  { id: 'payments', label: 'Payments', icon: 'creditCard', component: StudentPayments },
  { id: 'profile', label: 'My Profile', icon: 'user', component: StudentProfile },
];

const activeComponent = computed(() => {
  const tab = tabs.find(t => t.id === currentTab.value);
  return tab?.component || StudentOverview;
});

// Catch any child component error — prevent it from killing the whole dashboard
onErrorCaptured((err, instance, info) => {
  console.error('[StudentDashboard] Child component error:', err, info);
...
function fallbackNameFromSession(sessionUser, roleLabel = 'Student') {
...
/* Sidebar Promo */
.sidebar-promo { margin-top: 2rem; padding: 1.5rem 0 1rem; }
.promo-card {
  background: linear-gradient(135deg, #07193f 0%, #0d2859 100%);
  border-radius: 1rem; padding: 1rem; color: #ffffff;
...
.btn-promo {
  background-color: #ffc107; color: #0f172a; border: none; padding: 0.5rem 0.75rem;
  border-radius: 0.4rem; font-size: 0.7rem; font-weight: 800;
  cursor: pointer; width: 100%; position: relative; z-index: 2;
}
...
.search-box-desktop {
  ...
  border: 1px solid #eef2f6; border-radius: 0.75rem;
  padding: 0.65rem 1.25rem; width: 420px;
}
...
.btn-book {
  ...
  padding: 0.75rem 1.5rem; background-color: #0f172a; color: #ffffff;
  border-radius: 0.75rem; border: none; font-weight: 700; font-size: 0.85rem;
  cursor: pointer; transition: all 0.2s;
}
.btn-book:hover { background-color: #1e293b; transform: translateY(-1px); }
.btn-book-ic { color: currentColor; flex-shrink: 0; }
...
.dashboard-footer {
  height: 60px; background-color: #ffffff; border-top: 1px solid #f1f5f9;
  display: flex; align-items: center; justify-content: space-between;
  padding: 0 3rem; flex-shrink: 0;
}
...
@media (max-width: 768px) {
  ...
  /* Top Bar Mobile */
  .mobile-topbar {
    display: flex; background: #0f172a; padding: 1rem 1.25rem;
    align-items: center; justify-content: space-between;
    position: sticky; top: 0; z-index: 100;
    padding-top: max(1rem, env(safe-area-inset-top));
  }
  .topbar-logo { display: flex; align-items: center; }
  .mobile-logo-img { height: 80px; width: auto; filter: brightness(0) invert(1); margin-left: -10px; }
  .topbar-actions { display: flex; gap: 12px; align-items: center; }
  .icon-btn {
    width: 38px; height: 38px; background: rgba(255,255,255,0.1); border: none; border-radius: 50%;
